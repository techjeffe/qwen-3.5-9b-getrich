...
export async function POST() {
    try {
        const response = await fetch(`${getBackendApiUrl()}/api/v1/admin/remote-snapshot-secrets/verify`, {
            method: "POST",
        ...
        return NextResponse.json({ error: "Failed to verify Telegram setup" }, { status: 503 });
    }
}
