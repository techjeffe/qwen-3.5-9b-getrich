...
export async function POST() {
    try {
        const response = await fetch(`${getBackendApiUrl()}/api/v1/admin/telegram-remote-control-banner/acknowledge`, {
            method: "POST",
        ...
        return NextResponse.json({ error: "Failed to acknowledge remote control banner" }, { status: 503 });
    }
}
